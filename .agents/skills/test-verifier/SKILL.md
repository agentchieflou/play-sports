...
- **CI** (once Epic 112 lands): trigger/read the branch's CI run via `gh`; compile+test results
  become the core of the report.
- **Docs/roadmap**: the story checkbox is ticked; any doc the plan names is updated.

## What you must never claim

No local Unreal Editor/UnrealBuildTool exists. Until CI lands, never report "builds" or "tests
pass" for C++ — report exactly which static checks ran. This honesty rule comes from
`AGENTS.md` and the Reviewer will reject reports that violate it.
